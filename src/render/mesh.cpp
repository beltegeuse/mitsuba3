    ...
    if (unlikely(has_flag(ray_flags, RayFlags::BoundaryTest))) {
        ...
        if (!has_vertex_normals()) {
            ...
            Vector2f e0 = tp1 - tp0,
                     ...
                     v1 = p - tp1,
                     v2 = p - tp2;
            Vector2f pq0 = v0 - e0 * dr::clip(dr::dot(v0, e0) / dr::dot(e0, e0), 0, 1),
                     pq1 = v1 - e1 * dr::clip(dr::dot(v1, e1) / dr::dot(e1, e1), 0, 1),
                     pq2 = v2 - e2 * dr::clip(dr::dot(v2, e2) / dr::dot(e2, e2), 0, 1);
